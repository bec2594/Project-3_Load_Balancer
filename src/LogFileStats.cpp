...
LogFileStats::LogFileStats() :
    ...
    final_requests_in_queue(-1), // Initialize new member
    final_active_servers(-1), // Initialize new member
    final_inactive_servers(-1) // Initialize new member
{}
...
// New setter methods implementation
...
std::string LogFileStats::summary() const {
    ...
    // put new information right here
    if (final_requests_in_queue != -1) { // Only display if set
        ss << "Remaining Requests in Queue (at end): " << final_requests_in_queue << "\n";
    } else {
        ss << "Remaining Requests in Queue (at end): N/A (Data not provided)\n";
    }
    if (final_active_servers != -1) { // Only display if set
